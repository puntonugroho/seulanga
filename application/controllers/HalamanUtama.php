...
class HalamanUtama extends CI_Controller {
	...
	function __construct() {
        ...
        if(!in_array($this->session->userdata('group_id'), $this->session->userdata('kewenangan_dashboard'))) { redirect('keluar'); }
	...
	public function dashboard_modal_disposisi(){
		...
		if ($this->form_validation->run() == FALSE){ echo json_encode(array('st'=>0,'msg'=>'Anda Dilarang Melakukan Akses Langsung Ke Aplikasi')); return; }

		if($this->encrypt->decode(base64_decode($this->input->post('register_id')))=='-1'){ echo json_encode(array('st'=>0,'msg'=>'Anda Dilarang Melakukan Akses Langsung Ke Aplikasi')); return; }
