...
class ModelPenyitaan extends CI_Model {
	...
	private function _get_datatables_query(){
		...
		if($this->input->post('jenis_penyitaan')){ $this->db->where('jenis_penyitaan_id', $this->input->post('jenis_penyitaan')); }
		$this->db->from('register_penyitaan');
		if($_POST['search']['value']){
			$this->db->group_start();
			$this->db->like('jenis_penyitaan', $_POST['search']['value']);
	...
	public function get_seleksi_nomor_index($tahun_seleksi){
		try {
			...
			return $this->db->get('register_penyitaan');
		} catch (Exception $e) {
			return 0;
		}
	}
	...
	public function get_register_cetak($periode){
		try {
			return $this->db->query("SELECT * FROM register_penyitaan WHERE LEFT(tanggal_register,7)='$periode'");
		} catch (Exception $e) {
			return 0;
		}	
	}
